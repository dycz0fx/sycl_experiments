...
void add_data_to_buffer(packet_t pkt, size_t index, info_t info, sycl::atomic_ref<size_t, sycl::memory_order::acq_rel, sycl::memory_scope::device, sycl::access::address_space::ext_intel_global_device_space> tmp_counter_atomic) {
    ...
    //sycl::atomic_ref<size_t, sycl::memory_order::acq_rel, sycl::memory_scope::device, sycl::access::address_space::ext_intel_global_device_space> buff_atomic(info.buff[index%capacity]);
    ...
    //sycl::atomic_ref<int64_t, sycl::memory_order::acq_rel, sycl::memory_scope::device, sycl::access::address_space::ext_intel_global_device_space> buff_atomic(info.buff[0].flag);
    //buff_atomic.store(1);
...
#define pval(x) std::cout << #x " " << x << std::endl;
...
int main() {
    ...
    pval(last_mmap);  // unused
    pval(first_mmap); // RW
    pval(buff_mmap);  // RW
    pval(flag_mmap);  // RW
    pval(tmp_counter_mmap); //R debug
    pval(tmp_index_mmap);  // R debug
    pval(tmp_if_mmap);  // R debug
    pval(tmp_while_mmap);  // R debug
